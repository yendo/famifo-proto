...
いたが、ギャラリーはすべて認証の内側にあるため、この 302 は未認証を検知して
`/login` へ、`/login` は IdP の authorize エンドポイントへと連鎖する。DSM のセッションは
...
- **IdP がトークン交換を拒む** — 「IdP に届かない」とは別の失敗である。IdP は応答して
  おり、そのうえで断っている。`oidcauth.Exchange` は `*oauth2.RetrieveError` を
  `oidcauth.ProviderError`（HTTP ステータスと応答本文、1 KiB に切り詰め）に包んで返し、
  `errors.Is` で `oidcauth.ErrProviderRefused` と一致させて判別できるようにしてある。
  `internal/web` はこれを検知したときだけステータスと本文を ERROR でログに残し、
  訪問者には「サインインが拒まれた、もう一度試してほしい」を 503 で返す（502 は上流が
  不正な応答を返したときのものであり、ここでは上流は普通に応答している）。実機では
  ほとんどのログインは成功する中、`oauth2: "server_error"` とだけ言われて交換が失敗する
  ことが間欠的に起きている。再試行すると通る。

  **2026-09-11 に両側のログで挟み撃ちにして、失敗する箇所を一点に絞った。**

  ```
  18:51:38  DSM:    User [yendo] ... signed in to [DSM] successfully via [password]
  18:51:40  famifo: the identity provider refused the login  status=400  {"error":"server_error"}
  ```

  DSM 側は認証を成功として記録している。認可コードも発行されている。失敗しているのは
  その後のトークン交換の一手だけである。応答本文は `{"error":"server_error"}` の
  1 項目しかなく、`error_description` を持たない。nginx のエラーログにも何も残らない
  （CGI は落ちておらず、意図してこの応答を返している）。SSO Server 専用のログは無い。
  つまり **famifo 側から引き出せる情報は出し切っており、これ以上の計器を足しても増えない。**

  成功する回と失敗する回に、外から見える違いは見つかっていない。新規にパスワードを
  入力した回でも、IdP のセッションが生きたまま素通りした回でも、どちらでも成功も失敗も
  する。直前に成功があるかどうかとも関係しない。なお DSM は、パスワードを検証し直しても
  同じセッショントークンを使い回していた。関係するかは不明だが記録しておく。

  次に進むとしたら Synology の実装を読むかサポートに問い合わせることになる。運用上は
  再試行で回復する範囲の不具合であり、そこまでする価値があるとは判断していない。
