...
> **注記（2026-09-10）:** この計画は実行済みだが、Global Constraints のうち2つはもう
> 有効ではない。「新しい依存を入れない」は、ID トークンの検証を `go-oidc` + `x/oauth2`
> に移したことでもう事実ではない。「JWKS はログインのたびに取得する。キャッシュしない」
> も同様で、go-oidc の `RemoteKeySet` がキャッシュを持ち、未知の `kid` に出会ったときに
> 再取得する形に変わっている。経緯と現在の設計は
> `docs/superpowers/plans/2026-09-10-oidc-library-swap.md` と spec を見よ。
